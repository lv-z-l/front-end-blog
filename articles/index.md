...
<script setup>
  ...
  import { NTimeline, NTimelineItem, NTag, NSpace } from 'naive-ui'
...
<div id="main-page">
  <n-space :class="['main-page-space', show ? 'show' : 'hidden']">
    <n-tag v-for="tag in Object.keys(tagColors)" :class="[currentTag === tag ? 'current-tag' : '']" @click="filterArticleByTag(tag)" :color="{color: tagColors[tag]}" round>{{ tag }}</n-tag>
  </n-space>
        ...
        <template v-slot:header>
          <div class="color-fff">
            ...
            <n-space v-if="item.tags" style="display: inline-flex;">
              <n-tag v-for="tag in item.tags.split('、')" size="small" :color="{color: tagColors[tag]}" round>{{ tag }}</n-tag>
            </n-space>
          </div>
        </template>
